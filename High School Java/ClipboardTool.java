...
/**
 * @kali
 * purpose is to prevent looking a recently copied link, address or phone number 
...
public class ClipboardTool {
	...
	private static JFrame frame = new JFrame("ClipboardManager");
	private static JTextArea Tarea = new JTextArea(); // tarea = test area
	private static JScrollPane scroll = new JScrollPane(Tarea);
	...
	public static void main(String args[]) {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					out = new PrintWriter(new BufferedWriter(new FileWriter(
							customDir + "/ClipboardTool save file.txt")));
		...
		while (open_true = true) {
			frame.requestFocusInWindow();

			try {
				// CB = Toolkit.getDefaultToolkit().getSystemClipboard();
				transfer = CB.getContents(frame);
				currentCB = (String) transfer
											...
											// clipboard
			{
				...
				try {
					...
					Tarea.append((String) transfer
							.getTransferData(DataFlavor.stringFlavor) + "\n");
					System.out.println("current clipboard in text area: "
							+ currentCB);
	...
	public static void paste() { // unused methord, first try making this app
		...
		try {
			...
			currentCB = (String) transfer
					.getTransferData(DataFlavor.stringFlavor);
			System.out.println("current clipboard in the paste metord:"
					+ currentCB);
		} catch (Exception err) {
			err.printStackTrace();
