...
public class BattleShip {
	...
	public static boolean FULL_TEST = true;
	...
	public static Scanner input = new Scanner(System.in); // set up to get input
															// from console
		...
		for (int i = 0; i < boardSize; i++) {
			...
			for (int j = 0; j < boardSize; j++) {
				if (this.board[i][j] == 0) {
					...
					if (DEBUG)
						System.out.print(ANSI_BLUE + "S"); // print ship
															// location
					else
						System.out.print(" "); // print blank
				} else {
	...
	boolean placeShip(int size, int x, int y, boolean horizontal) {
		...
		if (horizontal) {
			for (int s = 0; s < size; s++)
				if (this.board[y][x + s] != 0)
					return false;
			for (int s = 0; s < size; s++)
				this.board[y][x + s] = shipNum;
		} else {
			for (int s = 0; s < size; s++)
				if (this.board[y + s][x] != 0)
					return false;
			for (int s = 0; s < size; s++)
	...
	int fireShot(int x, int y) {
		...
		if (this.board[y][x] < 0) {
			if (ship.sunk == 0)
				return -2; // firing on a sunk ship will just count as a miss
			return -1;
		}
	...
	int checkHitOrMiss(int x, int y) {
		if (x < 0 || y < 0 || x >= boardSize || y >= boardSize)
			return 2; // eliminate bad coordinates
		if (this.board[y][x] == 0)
			return 0; // empty spot
		if (this.board[y][x] == 1)
			return 1; // missed spot
		int shipNum = Math.abs(this.board[y][x]) - 2;
		Ship ship = this.ships.get(shipNum);
		if (this.board[y][x] < 0) {
			if (ship.sunk == 0)
	...
	public static void main(String[] args) {
		...
		if (FULL_TEST) {
			...
			for (int seed = 1; seed <= maxSeed; seed++) {
				...
				// game.printBoard();
				System.out.println(ANSI_GREEN + "Number of shots "
						+ game.numShots);
				total += game.numShots;
				if (maxS < game.numShots)
					maxS = game.numShots;
				if (minS > game.numShots)
					minS = game.numShots;
			}

			long stopTime = System.currentTimeMillis();
			long elapsedTime = stopTime - startTime;
			System.out.println("Average number of shots = " + (double) (total)
					/ maxSeed);
			System.out.println("Best number of shots = " + minS);
			System.out.println("Worst number of shots = " + maxS);
			System.out.println("Average time (ms) per game = "
					+ (double) (elapsedTime) / maxSeed);
		} else {
