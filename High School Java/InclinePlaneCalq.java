...
public class InclinePlaneCalq {
    ...
    public static void main(String[] args) {
        ...
        if (degrees >= 90) throw new ArithmeticException("First parameter is not valid");
        // System.out.println(s);

        System.out.println("please enter Mass:");
        double mass = scanner.nextDouble();
        if (mass <= 0) throw new ArithmeticException("Second parameter is not valid");

        System.out.println("please enter  μ:");
        double μ = scanner.nextDouble();
        if (μ < 0) throw new ArithmeticException("Third parameter is not valid");

        System.out.println("please select 0-5 decimal points you would like the answer in: ");
        int deci = scanner.nextInt();
        if (deci < 0 || deci > 5) throw new ArithmeticException("error with decimal points");
        ...
        bdfd = bdfd.setScale(deci, RoundingMode.HALF_UP);
        BigDecimal bdfu = new BigDecimal(fu);
        bdfu = bdfu.setScale(deci, RoundingMode.HALF_UP);
        BigDecimal bdfn = new BigDecimal(fn);
        bdfn = bdfn.setScale(deci, RoundingMode.HALF_UP);
        BigDecimal bda = new BigDecimal(a);
        bda = bda.setScale(deci, RoundingMode.HALF_UP);
