 ...
 * speed up seems not work work, likely is impossiblee
 *
 ...
 * the partical effects is not 100% my work either. used this video: https://www.youtube.com/watch?v=YCI8uqePkrc   and help from friend
 */
...
const height = canvas.height,// #E = MChammer^2... im not good at documenting
...
var platYArray = [1000, 800, 600, 400, 200, 1, -200, -400, -600]; // redundant to add value up here...
var speed = 90;
var speededUp = false;
//var shootArry = [];
...
var getNewXandY = function () {
    for (var i = 0; i < 10; i++) {
        ...
        ran.push(Math.floor(Math.random() * (width - 40) + 20)); // fill array with off set amount
    }//ran is used when calling platform methord
    ////////////////-------------//////////////
...
var plat = {
    ...
    makeReg: function (objXpos, objYpos) {
        ...
        ctx.beginPath();
        ctx.fillStyle = "blue";
        ctx.rect(objXpos - 25, objYpos - 10, 50, 20);//mid
        ctx.arc(objXpos + 25, objYpos, 10, 0, 2 * Math.PI);//right end
        ctx.fill();
        ctx.arc(objXpos - 25, objYpos, 10, 0, 2 * Math.PI);//left end
        ctx.fill();

    },
    makeVertMoving: function (objXpos, objYpos) { // hoz moving*
        ...
        this.objXpos = objXpos += this.moveSide;
        // if (this.objYpos < 200) {
        //  this.objYpos = objYpos;
        ...
        ctx.fillStyle = "yellow";
        ctx.rect(objXpos - 25, objYpos - 10, 50, 20);//mid
        ctx.arc(objXpos + 25, objYpos, 10, 0, 2 * Math.PI);//end
        ctx.fill();
        ctx.arc(objXpos - 25, objYpos, 10, 0, 2 * Math.PI);//end
        ctx.fill();
        //  this.moveUpY -= 10;
    ...
    hitCheck: function () {
        ...
        //check Y
            if (ball.goingDown && this.objYpos - ball.radi - 20 >= ball.Ypos - 10 && this.objYpos - ball.radi - 20 <= ball.Ypos + 10) { /////////////////////////
                ...
                // to comment out code to not play song(found at top)
                try {
                    audio.play();
                } catch (err) {
                }//catch
...
var gameOBJ = {
    ...
    pushDown: function () {
        if (pushDown <= -1300) {
            pushDown = 0;
            gameOBJ.nextLVL();// i like making methods and creating unnecessary overhead
            getNewXandY();
        }
    },
...
var game = function () {
    ...
    if (level == 10) {
        document.getElementById("lvlID").innerHTML = "The Doodle successfully escaped mount doom!!... JK :)"
    }
    ...
    ball.draw();//draw here so "ball" is 2nd obj to top
